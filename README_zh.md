...
翻译网页上的图片，可以一次翻译一整页的全部图片，也可以只翻译单张图片或者框选的部分。支持各种图片的翻译，针对漫画、条漫、同人本进行了优化。
...
* **截屏翻译 / OCR** —— 在页面上框选任意区域识别并翻译其中的文字，识别出的文字可以直接复制。
* **拍照翻译** —— 用摄像头对准现实中的文字，翻译看到的内容。
* **自动翻译** —— 自动翻译当前视口内的图片，不会改动页面其它部分。（若开启自动滚动，则会自动向下滚动页面，逐一翻译页面上的所有图片。）
* **朗读（TTS）** —— 朗读识别出的原文和/或译文，可单条朗读，也可连续朗读。
* **语言学习辅助** —— 为中文原文添加拼音、为日文原文添加振假名（furigana）标注。
* **翻译缓存与阅读器** —— 在本地保存原图、译图和文字框数据，复用缓存结果避免重复翻译，也可以在自带的阅读器里回看或导出为 ZIP。
...
2. 打开 ImageTrans，点击菜单栏 **工具 → 服务器** 打开服务器程序，点击 **启动服务器** 按钮，然后点击 **重新连接** 按钮。
...
* **屏幕截图 OCR**（**CTRL + SHIFT + L**）—— 框选一个区域即可识别/翻译，识别出的文字可复制、朗读，或在弹出窗口里新建区域继续识别。
* **拍照翻译** —— 允许摄像头权限后翻译摄像头看到的内容。
* **开始/停止自动翻译** —— 自动翻译当前视口内的图片。可以开启自动滚动以翻译所有图片。
...
* **自动化** —— 自动翻译时自动滚动到下一张未翻译的图片。
* **悬浮翻译按钮** —— 在页面上显示一个可拖拽的翻译按钮，并可配置点击它时执行的操作。
