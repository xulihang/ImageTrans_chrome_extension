...
var LocalOCR = (function () {
    ...
    const MAX_DECODER_STEPS = 128;  // Most manga text lines are under 50 chars; 128 is a safe upper bound
    ...
    async function initialize(progressCallback) {
        ...
        ortGlobal.env.wasm.wasmPaths = extUrl("lib/");
        ortGlobal.env.wasm.numThreads = 1;

        if (progressCallback) progressCallback("Loading vocabulary...");
        await loadVocab();

        var yoloUrl = extUrl("yolov8-lines-model/model.onnx");
        ...
        var results = await Promise.all([
            ortGlobal.InferenceSession.create(yoloUrl),
            ortGlobal.InferenceSession.create(encoderUrl),
            ortGlobal.InferenceSession.create(decoderUrl)
        ]);
